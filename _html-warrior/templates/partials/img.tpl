{strip}
  ...
  if ( !isset($width) && !isset($height) ) {
    $full_image_path = $htmlwarrior->config["basepath"] . "/" . $htmlwarrior->runtime["site_dir"] . "/images/".$src;
  ...
  {/if}
    <img src="images/{$src}" alt="{$alt}" title="{$alt}" {if $class}class="{$class}" {/if}{if $id}id="{$id}" {/if}{if !$nodimension}{if isset($width)}width="{$width}" {/if}{if isset($height)}height="{$height}" {/if}{/if}{if $align}style="float: {$align}" {/if}{if isset($border)}border="{$border}" {/if}/>
  {if isset($href) || isset($onclick)}
    </a>
  {/if}
  ...
  {php}
    ...
    $smarty->clearAssign("src");
    $smarty->clearAssign("file");
  {/php}
{/strip}
...
require $config_q["code_path"].'/init.php';
require $config_q["code_path"].'/externals/smarty/libs/Smarty.class.php';
require_once($config_q["code_path"].'/filesystem.php');
...
// admin
if($site_dir=="") {
	$smarty->template_dir =$config_q["code_path"]."/admin/templates/";
	$smarty->assign("access_log", get_access_log(array("limit"=>10)));

	$files = dir_list( $config_q["basepath"] );
	foreach($files as &$ma)
    $tmp[] = &$ma["timestamp"];
...
if(isset($_GET["build"])) {
	if($_GET["build"]==1) {
		require_once($config_q["code_path"].'/build.php');
...
if(!file_exists($config_q["basepath"]."/".$smarty->template_dir."/pages/".$page.".tpl") && !strpos($request_uri[1],"__logged")) { 
	echo '<div style="background: red">Templatet ei ole olemas. Kopeerin uue?. <a href="?copy=yes">jah</a></div>';
	if(@$_GET["copy"]=="yes") { 
		if (copy( $config_q["code_path"]."/templates/pages/".$page.".tpl" , $config_q["basepath"]."/".$smarty->template_dir."pages/".$page.".tpl" )) { 
			echo "done";
		} else { 
			echo $config_q["code_path"]."/templates/pages/".$page.".tpl does not exist!";
		}
	}
	die();
}
...
if(@strpos($request_uri[1],"__logged")) { 
	...
	$template_filetime = filemtime($config_q["basepath"]."/$site_dir/templates/pages/".str_replace("__logged", "", $page).".tpl");
} else {
	$smarty->assign("logged_sufix", "");
	$smarty->assign("logged", false);
	$page_content = $smarty->fetch("pages/".$page.".tpl");
	$template_filetime = filemtime($config_q["basepath"]."/$site_dir/templates/pages/".$page.".tpl");
}
...
build_template($config_q["basepath"]."/$site_dir/".$config["build_dir"]."/".$page.".html", $content, $template_filetime);
